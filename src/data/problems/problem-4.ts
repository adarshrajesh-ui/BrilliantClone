...
export const PROBLEM_4: ProblemDefinition = {
  ...
  scenarioText:
    ...
    'The value, count, and probability for each group are given. Fill the contribution (value × probability) for each, then the expected value of the hand.',
  ...
  mistakeRules: [
    ...
    { mistakeType: 'unweighted-sum', feedback: 'You added the raw card values (2 + 4 + 10 = 16). Weight by probability first: 0.5 + 1.0 + 5.0 = 6.5.' },
    { mistakeType: 'arithmetic-error', feedback: 'Check each row: contribution = value × probability, then add them up.' },
  ],
  feedback: { correct: 'Correct! 2×1/4 = 0.5, 4×1/4 = 1.0, 10×1/2 = 5.0, so EV = 6.5.' },
  teachingExplanation: {
    title: 'Why this makes sense',
    body: [
      'Group the hand by card value. Each value group contributes value × probability to the average draw — the contribution column — and the expected value is the sum of those contributions.',
  ...
  hints: [
    { id: 'p4-h1', label: 'Weight each value', content: 'Contribution = value × probability. Value 2 × 1/4 = 0.5.' },
    { id: 'p4-h2', label: 'Add contributions', content: 'EV = 0.5 + 1.0 + 5.0 = 6.5.' },
  ],
  completionRule: 'Fill all three contributions and submit the final expected value.',
